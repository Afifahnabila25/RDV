...
try:
    ...
    print("[INFO] XGBoost tidak tersedia, menggunakan GradientBoosting sebagai pengganti.")
...
if not DB_PATH.exists():
    ...
    if not DB_PATH.exists():
        raise FileNotFoundError(f"Database tidak ditemukan. Pastikan pipeline sudah dijalankan.")
...
def main():
    ...
    for i, row in df_zone.iterrows():
        print(f"      {i+1}. {row['zone_name']} ({row['borough']})"
