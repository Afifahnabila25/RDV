...
st.markdown("""
    ...
    const observer = new MutationObserver(() => {
        ...
        maps.forEach(map => {
            if (!map.dataset.doubleClickAttached) {
                map.dataset.doubleClickAttached = "true";
                map.addEventListener('dblclick', function(e) {
                    // Simulasi trigger tombol zoom-in bawaan plotly saat di-double click
                    const zoomInBtn = map.querySelector('[data-title="Zoom in"]');
                    if (zoomInBtn) {
                        zoomInBtn.click();
                        zoomInBtn.click(); // Dua kali klik untuk efek zoom lebih terasa
                    }
                });
            }
        });
...
with col1:
    with st.container(border=True):
        ...
        st.caption("Sekarang kamu bisa double-click langsung di area peta untuk memperbesar (zoom in).")
        ...
        try:
            ...
            df_map_data.columns = [col.lower() for col in df_map_data.columns]
            ...
            default_coords = {
                ...
                "latitude": [40.6895, 40.8653, 40.7260, 40.7644, 40.6771, 40.7077, 40.6260, 40.7990, 40.6853, 40.6960, 40.8079, 40.8223, 40.7291, 40.6413, 40.7769, 40.7641, 40.7684, 40.7731, 40.7770, 40.7562, 40.7561, 40.7621, 40.7497, 40.7479, 40.6698, 40.7580, 40.7359, 40.7781, 40.7719, 40.7912, 40.7812, 40.8223, 40.7829, 40.7601, 40.5110, 40.8003],
                "longitude": [-74.1745, -73.8474, -73.9790, -73.9235, -73.7844, -74.0175, -74.0301, -73.9665, -73.9840, -73.9933, -73.9458, -73.9390, -73.9866, -73.7781, -73.8740, -73.9567, -73.9602, -73.9818, -73.9865, -73.9785, -73.9715, -73.9786, -73.9842, -73.9757, -73.9805, -73.9855, -73.9904, -73.9542, -73.9588, -73.9729, -73.9772, -73.9390, -73.9654, -73.9912, -74.2403, -73.9348]
            }
            ...
            if not df_geo.empty:
                ...
                st.plotly_chart(
                    ...
                    config={
                        "scrollZoom": True,
                        "doubleClick": False, # Nonaktifkan perilaku reset default Plotly
                        "displayModeBar": True # Memunculkan bar navigasi tersembunyi agar dibaca oleh skrip otomatis dblclick
                    }
                )
            else:
                st.info("Menunggu sinkronisasi data wilayah...")
                
        except Exception as e:
            st.error(f"Gagal memuat visualisasi peta koordinat: {e}")

with col2:
    with st.container(border=True):
        st.subheader("💰 Total Pendapatan per Wilayah")
        st.caption("Distribusi total pendapatan berdasarkan wilayah (Borough).")
        ...
        try:
            ...
            if not df_borough.empty:
                df_borough.columns = [col.lower() for col in df_borough.columns]
                ...
                st.info("Data pendapatan wilayah tidak ditemukan.")
                
        except Exception as e:
            st.error(f"Gagal memuat data pendapatan borough: {e}")
